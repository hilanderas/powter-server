...
class InfoMgr:
    ...
    def mergeinfo(self):
        ...
        dnsfile, sskcpfile = self.args["dns"], self.args["sskcp"]
        validated = val(dnsfile) and val(sskcpfile)
        if validated:
            parse, write = self.yamlwrapper.parse, self.yamlwrapper.write_yaml
            dnsinfo, sskcpinfo = parse(dnsfile), parse(sskcpfile)
            info = {"dns-proxy": dnsinfo, "sskcp": sskcpinfo}
    ...
    def divideinfo(self):
        if self.validate():
            ...
            write(content["dns-proxy"], self.args["dns"]) 
